...
1. **Add entry with note**:
   ...
   - Save and verify note appears in list between value and timestamp

2. **Add entry without note**:
   - Add entry with value "100"
   - Leave note field blank
   - Save and verify only value and timestamp shown in list (no blank space)

3. **Edit existing entry - add note**:
   - Tap entry that has no note
   - Add note text
   - Save and verify note now appears in list

4. **Edit existing entry - modify note**:
   - Tap entry that has a note
   - Verify note field shows existing note
   - Modify note text
   - Save and verify updated note appears in list

5. **Edit existing entry - remove note**:
   - Tap entry with note
   - Clear note field (blank)
   - Save and verify note no longer appears in list

6. **Long note truncation**:
   - Add entry with very long note (100+ characters)
   - Verify list shows ellipsis truncation
   - Verify full note visible when editing

7. **Process death simulation**:
   - Start adding entry with value and note
   - Rotate device (configuration change)
   - Verify value and note are preserved

8. **Backward compatibility - Import old backup**:
   - If possible, use v0.5.0 app to export data
   - Import into v0.6.0 app
   - Verify entries load successfully with note = null

9. **Export with notes**:
   - Export data that includes entries with notes
   - Inspect JSON file to verify note field is present
   - Re-import and verify notes are restored
